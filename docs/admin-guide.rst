...
Edit ``/etc/synnefo/20-snf-astakos-app-settings.conf`` and set the corresponding variables 
``ASTAKOS_TWITTER_TOKEN`` and ``ASTAKOS_TWITTER_SECRET`` to reflect your newly created pair. 
...
Edit ``/etc/synnefo/20-snf-astakos-app-settings.conf`` and set the corresponding variables 
``ASTAKOS_GOOGLE_CLIENT_ID`` and ``ASTAKOS_GOOGLE_SECRET`` to reflect your newly created pair. 
